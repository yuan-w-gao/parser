...
namespace shrg::em{
VITERBI_HMM::VITERBI_HMM(RuleVector &shrg_rules, std::vector<EdsGraph> &graphs, Context *context,
               double threshold, std::string dir, const std::string& validationDir)
    : EM_VITERBI(shrg_rules, graphs, context, threshold, dir) {
...
void VITERBI_HMM::log_matrices(const string& log_file) {
...
void VITERBI_HMM::log_iteration_info(int iteration, double ll, double entropy, double kl,double hmm_entropy, double hmm_kl, double time_diff, const string& log_file) {
    ofstream log_stream(log_file, ios_base::app); // Append mode
    if (!log_stream) {
        cerr << "Error opening log file: " << log_file << endl;
        return;
...
void VITERBI_HMM::initialize_true(LabelToRule& dict) {
...
vector<vector<double>> VITERBI_HMM::ind_to_matrix(const vector<vector<int>> transition, size_t n, size_t m){
...
vector<double> VITERBI_HMM::ind_to_vector(vector<int> prob_ind){
...
void VITERBI_HMM::print_matrix_norm(){
    vector<vector<double>> transitions = ind_to_matrix(transition_ind, transition_ind.size(), transition_ind[0].size());
    vector<vector<double>> emissions = ind_to_matrix(emission_ind, emission_true.size(), emission_true[0].size());
...
double VITERBI_HMM::evaluate_cross_entropy() {
...
double VITERBI_HMM::evaluate_kl() {
    double total_kl = 0.0;
