...
int main(int argc, char *argv[]) {
    ...
    std::string out_dir = std::string(argv[4]) + "viterbi_em/";

    // shrg::em::BatchEM model = shrg::em::BatchEM(shrg_rules, graphs, context, threshold, 50,argv[4], 5);
    shrg::em::ViterbiEM model = shrg::em::ViterbiEM(shrg_rules, graphs, context, threshold, out_dir, 5);
    model.run();
